...
int main() {
	...
	for (int g = 0; g < n; ++g) {
		MATR watts = watts_strogatzGraph(n, g , 0.5);
		for (int q = 0; q <= 1000; q = q + 1) {

			vector<bool> erased(n, false);
			if (nodos) percolacionNodos(erased, (float)q / 1000.0);
			else percolacionAristas(watts, (float)q / 1000.0);
			bool res = testConnex(watts, erased);
			if (res) {
				resultats[g][q] = 1;
			}
			else {
			...
			if (!nodos)  desPercolacionAristas(watts);
		}
	}
